...
class TemplateQuoteRequestAPITests(APITestCase):
    ...
    def test_calculate_price_endpoint(self):
        ...
        url = f"/api/templates/templates/{self.template.id}/calculate_price/"
        data = {
            "quantity": 250,
            "selected_options": [self.qty_250.id],
            "selected_finishing": []
        }
        response = self.client.post(url, data, format="json")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertIn("calculated_price", response.data)
...
class TemplatePriceCalculationTests(TestCase):
    ...
    def test_upgraded_options_calculation(self):
        ...
        # Mandatory finishing (already in base)
        # total += self.lam.price_adjustment  # 0
